...
pub async fn publish(
    ...
    let exists = sqlx::query!(
        "SELECT * FROM v1.trunk_project WHERE name = $1",
        new_extension.name
    )
    ...
    match exists {
        ...
        Some(exists) => {
            ...
            match version_exists {
                ...
                None => {
                    ...
                    sqlx::query!(
                        "
                    INSERT INTO v1.extension_versions(extension_id, version, created_at, yanked, license, published_by, extension_name, system_dependencies, libraries)
        ...
        None => {
            ...
            let id_row = sqlx::query!(
                "
            INSERT INTO v1.trunk_project(name, created_at, updated_at, description, homepage, documentation, repository)
            ...
            sqlx::query!(
                "
            INSERT INTO v1.extension_versions(extension_id, num, created_at, yanked, license, published_by, extension_name, system_dependencies, libraries)
            VALUES ($1, $2, (now() at time zone 'utc'), $3, $4, $5, $6, $7::jsonb, $8::jsonb)
            ",
                extension_id as i32,
                new_extension.vers.to_string(),
    ...
    Ok(HttpResponse::Ok().body(format!(
        "Successfully published extension {} version {}use crate::views::extension_publish::ExtensionUpload;
use crate::views::user_info::UserInfo;
use actix_multipart::Multipart;
use actix_web::http::header::AUTHORIZATION;
use actix_web::{delete, error, get, post, web, HttpResponse};
...
pub async fn get_version_history(
    ...
    let rows = sqlx::query!(
        "SELECT * FROM v1.extension_versions WHERE extension_id = $1",
        extension_id
    )
    .fetch_all(&mut tx)
    .await?;
    ...
    for row in rows.iter() {
        ...
        v1.extension_versions.push(data);
    }
...
pub async fn get_version(
    ...
    let mut tx = conn.begin().await?;
    // Get extension information
    let row = sqlx::query!("SELECT * FROM v1.trunk_project WHERE name = $1", name)
        .fetch_one(&mut tx)
        .await?;
    ...
    // Get information for all versions of extension
    let rows = sqlx::query!(
        "SELECT * FROM v1.extension_versions WHERE extension_id = $1 AND version = $2",
        extension_id,
        version
    )
    .fetch_all(&mut tx)
